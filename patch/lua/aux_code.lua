...
function AuxFilter.init(env)
    ...
    env.avoid_prefixes = {}
    local avoid_prefixes_cfg = config:get_list("aux_code/avoid_prefixes")
    if avoid_prefixes_cfg and avoid_prefixes_cfg.size > 0 then
        for i = 0, avoid_prefixes_cfg.size - 1 do
            table.insert(env.avoid_prefixes, avoid_prefixes_cfg:get_value_at(i).value)
        end
    end
...
function AuxFilter.func(input, env)
    ...
    for _, prefix in ipairs(env.avoid_prefixes) do
        if string.starts(inputCode, prefix) then
